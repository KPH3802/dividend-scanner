...
def run_scan(lookback_days=None):
    ...
    for cut in cuts:
        ...
        _ns = scored['net_score']
        _fired = 1 if _ns >= 3 else 0
        _bucket = str(min(_ns, 5)) + '+' if _ns >= 5 else str(_ns)
        log_signal_intelligence(scored['declaration_date'], 'DIV_CUT', scored['ticker'], 'BUY',
                                _fired, signal_strength=float(_ns), signal_bucket=_bucket,
                                score=_ns if _fired else None)
